...
void parseCmdInput(Disc* disc, char* cmd, void* data) {
    ...
    switch (i) {
        ...
        case 2:
            ...
            break;
        case 3:
            if (strcmp(argv[0], "R") != 0 || strcmp(argv[0], "W") != 0) {
                ...
                writeOp_client(disc, index, len, data);
            }
            break;
...
char* readOp(Disc* disc, int cylinderID, int sectorID) {
    char* buf = malloc((BLOCK_SIZE + 1) * sizeof(char));
...
char* readOp_client(Disc* disc, int blockID) {
    return readOp(disc, blockID / disc->_numSectorPerCylinder, blockID % disc->_numSectorPerCylinder);
}
